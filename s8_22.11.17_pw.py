...
# Three Tracker
#----------------------------------------------------------------------------------------------------------------    
def three_tracker(k, cropped_mask, boxes, raw_path):
   ...
   #problem area
    ...
    for i in tqdm(range(len(point))): 
        ...
        events[k + i] = np.array([p_area,np.max(p_area),p_cent,len(regionprops(particle_mask)),p_amp,np.max(p_amp)-np.min(p_amp),first_frame,p_region[0],
...
for filename in os.listdir(folder_path):
    ...
    if filename.endswith('.tif') and os.path.isfile('{0}/{1}'.format(folder_path,filename)):
        ...
        for k,v in cropped_values[0].items():
            sites.update(three_tracker(k, v,cropped_values[1], raw_path))
